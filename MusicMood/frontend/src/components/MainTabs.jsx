...
import { Container, Row, Col, Tabs, Tab, Alert, Form, Button, InputGroup, Spinner } from "react-bootstrap";
    ...
    const loadContent = useCallback(async (targetKey, mood = activeMood) => {
        ...
        try {
            ...
            if (targetKey === 'mood') res = await api.getTracksByMood(mood);
                ...
                res = await api.getTrackByTime();
                setTimeOfDay(res.data.time_of_day);
                res.data = res.data.tracks; //приводим к общему формату
            }
    ...
    return (
        <Container className="py-4">
            ...
            <Form onSubmit={handleSearch} className="mb-4">
                <InputGroup size="lg">
                    <Form.Control placeholder="Найти трек..." value={searchQuery}
            ...
            {key === 'results' ? (
                ...
                <Tabs activeKey={key} onSelect={setKey} className="nav-pills justify-content-center">
                    <Tab eventKey='mood' title='По настроению'>
                        <div className="f-flex justify-content-center gap-2 my-3 flex-wrap">
                            {MOODS.map(m => (
                                <Button key={m.key} variant={activeMood === m.key ? 'primary' : 'outline-primary'}
                                onClick={() => {setActiveMood(m.key); loadContent('mood', m.key);}}>
                                    {m.icon} {m.name}
                                </Button>
                            ) )}
                        </div>
